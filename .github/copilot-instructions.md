...
- The main data flow is: `scripts/collection/collect_crossref.py` / `scripts/collection/kaggle_collect_openalex_sri_lanka.py` -> collector classes in `src/collectors/` -> normalization/preprocessing in `src/preprocessing/` -> JSONL/CSV data under `data/`.
...
- Output conventions are simple and file-based: JSONL is the intermediate format for collection scripts, while `scripts/processing/jsonl_to_csv.py` and `scripts/quality/compare_dois.py` convert or compare datasets.
