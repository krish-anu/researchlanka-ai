"""Structured Phase 3 (overall application) test result reporting.

Writes:
  - CSV + Markdown + JSON suite summary
  - Security matrix: Test ID | Endpoint | Scenario | Expected | Result
"""
...
DEFAULT_REPORT_DIR = (
    Path(__file__).resolve().parents[2] / "data" / "reports" / "phase3_overall_tests"
)
...
def infer_phase3_category(item: Any) -> str:
    marker_names = {marker.name for marker in getattr(item, "iter_markers", lambda: [])()}
    for category in PHASE3_CATEGORIES:
        if category in marker_names:
            return category

    nodeid = str(getattr(item, "nodeid", "")).lower()
    mapping = {
        "frontend": ("frontend", "vitest", "middleware", "next.config"),
        "integration": ("integration", "end_to_end", "smoke_flow"),
        "performance": ("performance", "latency", "budget"),
        "security": ("security", "sql_injection", "secret", "https", "exposure"),
        "load": ("load", "concurrent", "throughput"),
        "recovery": ("recovery", "failover", "unreachable", "corrupt"),
        "accessibility": ("accessibility", "a11y"),
        "deploy_config": ("deploy", "compose", "docker"),
        "deployed": ("deployed_site", "live_base", "deployed"),
    }
...
def write_phase3_report(
    ...
    *,
    security_matrix: list[SecurityMatrixRow] | None = None,
    report_dir: Path | None = None,
    suite_name: str = "phase3_overall",
) -> dict[str, Path]:
    ...
    matrix_csv = out_dir / f"{base}_security_matrix.csv"
    matrix_md = out_dir / f"{base}_security_matrix.md"

    latest_csv = out_dir / "latest_results.csv"
    latest_md = out_dir / "latest_report.md"
    latest_json = out_dir / "latest_summary.json"
    latest_matrix_csv = out_dir / "latest_security_matrix.csv"
    latest_matrix_md = out_dir / "latest_security_matrix.md"
        ...
        category_rows.append(
            {
                ...
                "skipped": cat_skipped,
                "pass_rate_pct": round(
                    (cat_passed / len(subset) * 100.0) if subset else 0.0, 2
                ),
            }
        )

    matrix = list(security_matrix or [])
    matrix_summary = {
        "total": len(matrix),
    ...
    summary = {
        ...
        "security_matrix": matrix_summary,
        "failed_tests": [
    ...
    if not matrix:
        matrix_lines.append("| — | — | No security matrix cases recorded | — | — |")
    ...
    return {
        ...
        "markdown": md_path,
        "json": json_path,
        "security_matrix_csv": matrix_csv,
        "security_matrix_markdown": matrix_md,
        "latest_csv": latest_csv,
        "latest_markdown": latest_md,
        "latest_json": latest_json,
        "latest_security_matrix_csv": latest_matrix_csv,
        "latest_security_matrix_markdown": latest_matrix_md,
    }
