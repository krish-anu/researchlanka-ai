...
_REAL = load_real_publications(limit=8)
_SAMPLE_TOKEN = next(
    (
        token
        for token in str(_REAL[0].get("title") or "research").replace(",", " ").split()
...
def test_health_meta_list_detail_flow(api: ResearchLankaAPI):
    ...
    listing = route_get(api, "/api/v1/publications", {"q": [_SAMPLE_TOKEN]})
    assert listing["data"]
    key = listing["data"][0]["publication_key"]
    assert key != "doi:10.1000/test"
...
def test_search_suggest_facets_and_analytics_chain(api: ResearchLankaAPI):
    suggest = route_get(api, "/api/v1/search/suggest", {"q": [_SAMPLE_TOKEN]})
    assert suggest["data"]
