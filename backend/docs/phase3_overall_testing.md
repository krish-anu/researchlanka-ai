...
5. Submittable Word report regenerated:
   `python scripts/testing/generate_standard_test_report_doc.py`
   → `data/processed/common/ResearchLanka_Standard_Test_Report.doc`
...
- **TODO:** Live Postgres latency/load soak *(set reachable `DATABASE_URL` in `.env` and run soak against `/api/v1/health` + hot list endpoints)*.
- **TODO:** Browser E2E (Playwright) is not yet in CI *(add smoke for login, search, publication detail, admin gate)*.
- **TODO:** Live deployed-site HTTP suite *(re-run from a host that can open `RESEARCHLANKA_DEPLOYED_BASE_URL`, currently `http://98.81.141.112:3000`)*.
- **TODO:** Full frontend Vitest when skipped *(unset `PHASE3_SKIP_VITEST` and run `cd frontend && npm test`)*.
