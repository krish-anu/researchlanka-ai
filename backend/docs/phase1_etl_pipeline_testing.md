...
## Outstanding (mark in the Word report)

- **TODO:** Full Dagster `dg/dev` materialization *(run `uv run dg dev`, materialize `researchlanka_all_assets_job` or a subset, archive run IDs)*.
- **TODO:** Package-local `backend/dagster-quickstart/tests` *(add tests there or accept `backend/tests/phase1` coverage)*.
